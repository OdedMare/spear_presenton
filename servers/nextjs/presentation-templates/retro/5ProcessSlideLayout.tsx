...
const ProcessSlideLayoutComponent = ({ data }: { data: z.infer<typeof Schema> }) => {
    ...
    return (
        <div className="w-full max-w-[1280px] aspect-video mx-auto relative overflow-hidden rounded-md h-full flex flex-col justify-center p-12" style={{ backgroundColor: '#fef3c7', color: '#451a03' }}>
            <div className="flex justify-between items-start relative">
                {/* Connecting Line */}
                <div className="absolute top-8 left-0 w-full h-2 -z-10" style={{ backgroundColor: '#fde68a' }} />
                ...
                {data.steps?.map((step, idx) => (
                    <div key={idx} className="flex flex-col items-center w-64 text-center">
                        <div className="w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold mb-4 z-10 border-4" 
                             style={{ backgroundColor: '#b45309', color: '#fef3c7', borderColor: '#fef3c7' }}>
                            {idx + 1}
                        </div>
                        <h3 className="text-xl font-bold mb-2" style={{ color: '#d97706' }}>{step.title}</h3>
