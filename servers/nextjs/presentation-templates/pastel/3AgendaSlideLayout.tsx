...
const AgendaSlideLayoutComponent = ({ data }: { data: z.infer<typeof Schema> }) => {
    ...
    return (
        <div className="w-full max-w-[1280px] aspect-video mx-auto relative overflow-hidden rounded-md h-full p-12" style={{ backgroundColor: '#fdf2f8', color: '#831843' }}>
            <h2 className="text-4xl font-bold mb-12 border-b-2 pb-4" style={{ borderColor: '#fbcfe8', color: '#db2777' }}>{data.title}</h2>
            <div className="grid grid-cols-1 gap-6 max-w-3xl mx-auto">
                {data.items?.map((item, idx) => (
                    <div key={idx} className="flex items-center p-6 rounded-xl shadow-sm" style={{ backgroundColor: 'rgba(255,255,255,0.5)' }}>
                        <span className="text-3xl font-bold mr-6 w-12 h-12 flex items-center justify-center rounded-full" style={{ backgroundColor: '#db2777', color: '#fdf2f8' }}>{idx + 1}</span>
