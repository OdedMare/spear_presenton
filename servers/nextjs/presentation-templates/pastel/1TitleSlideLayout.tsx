...
const TitleSlideLayoutComponent = ({ data }: { data: z.infer<typeof Schema> }) => {
    ...
    return (
        <div className="w-full max-w-[1280px] aspect-video mx-auto relative overflow-hidden rounded-md h-full flex flex-col items-center justify-center p-12 text-center" style={{ backgroundColor: '#fdf2f8', color: '#831843' }}>
            {image && (
            ...
            <div className="z-10 relative max-w-4xl space-y-6">
                <h1 className="text-6xl font-bold tracking-tight" style={{ color: '#db2777' }}>{title}</h1>
                <p className="text-2xl opacity-90" style={{ color: '#be185d' }}>{subtitle}</p>
                <div className="w-32 h-2 rounded-full mx-auto mt-8" style={{ backgroundColor: '#fbcfe8' }} />
            </div>
        </div>
    );
