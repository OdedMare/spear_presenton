...
const TeamSlideLayoutComponent = ({ data }: { data: z.infer<typeof Schema> }) => {
    ...
    return (
        <div className="w-full max-w-[1280px] aspect-video mx-auto relative overflow-hidden rounded-md h-full p-12" style={{ backgroundColor: '#09090b', color: '#e4e4e7' }}>
            <h2 className="text-4xl font-bold mb-12 text-center" style={{ color: '#d946ef' }}>{data.title}</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                {data.members?.map((member, idx) => (
                    <div key={idx} className="flex flex-col items-center text-center p-6 rounded-lg" style={{ backgroundColor: '#22d3ee', opacity: 0.9 }}>
                        <div className="w-32 h-32 rounded-full mb-4 overflow-hidden bg-gray-300 border-4" style={{ borderColor: '#d946ef' }}>
                            {member.image ? (
                        ...
                        <h3 className="text-xl font-bold" style={{ color: '#e4e4e7' }}>{member.name}</h3>
                        <p className="text-md opacity-75" style={{ color: '#8b5cf6' }}>{member.role}</p>
                    </div>
                ))}
            </div>
        </div>
