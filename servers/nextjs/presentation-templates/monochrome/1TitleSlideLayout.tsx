...
const TitleSlideLayoutComponent = ({ data }: { data: z.infer<typeof Schema> }) => {
    ...
    return (
        <div className="w-full max-w-[1280px] aspect-video mx-auto relative overflow-hidden rounded-md h-full flex flex-col items-center justify-center p-12 text-center" style={{ backgroundColor: '#ffffff', color: '#09090b' }}>
            {image && (
            ...
            <div className="z-10 relative max-w-4xl space-y-6">
                <h1 className="text-6xl font-bold tracking-tight" style={{ color: '#18181b' }}>{title}</h1>
                <p className="text-2xl opacity-90" style={{ color: '#52525b' }}>{subtitle}</p>
                <div className="w-32 h-2 rounded-full mx-auto mt-8" style={{ backgroundColor: '#d4d4d8' }} />
            </div>
        </div>
    );
