...
const ProcessSlideLayoutComponent = ({ data }: { data: z.infer<typeof Schema> }) => {
    ...
    return (
        <div className="w-full max-w-[1280px] aspect-video mx-auto relative overflow-hidden rounded-md h-full flex flex-col justify-center p-12" style={{ backgroundColor: '#000000', color: '#ffffff' }}>
            <div className="flex justify-between items-start relative">
                {/* Connecting Line */}
                <div className="absolute top-8 left-0 w-full h-2 -z-10" style={{ backgroundColor: '#ffffff' }} />
                ...
                {data.steps?.map((step, idx) => (
                    <div key={idx} className="flex flex-col items-center w-64 text-center">
                        <div className="w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold mb-4 z-10 border-4" 
                             style={{ backgroundColor: '#facc15', color: '#000000', borderColor: '#000000' }}>
                            {idx + 1}
                        </div>
                        <h3 className="text-xl font-bold mb-2" style={{ color: '#eab308' }}>{step.title}</h3>
