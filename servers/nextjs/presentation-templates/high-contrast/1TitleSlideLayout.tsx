...
const TitleSlideLayoutComponent = ({ data }: { data: z.infer<typeof Schema> }) => {
    ...
    return (
        <div className="w-full max-w-[1280px] aspect-video mx-auto relative overflow-hidden rounded-md h-full flex flex-col items-center justify-center p-12 text-center" style={{ backgroundColor: '#000000', color: '#ffffff' }}>
            {image && (
            ...
            <div className="z-10 relative max-w-4xl space-y-6">
                <h1 className="text-6xl font-bold tracking-tight" style={{ color: '#facc15' }}>{title}</h1>
                <p className="text-2xl opacity-90" style={{ color: '#eab308' }}>{subtitle}</p>
                <div className="w-32 h-2 rounded-full mx-auto mt-8" style={{ backgroundColor: '#ffffff' }} />
            </div>
        </div>
    );
