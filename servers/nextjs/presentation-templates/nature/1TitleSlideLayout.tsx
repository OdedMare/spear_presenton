...
const TitleSlideLayoutComponent = ({ data }: { data: z.infer<typeof Schema> }) => {
    ...
    return (
        <div className="w-full max-w-[1280px] aspect-video mx-auto relative overflow-hidden rounded-md h-full flex flex-col items-center justify-center p-12 text-center" style={{ backgroundColor: '#f0fdf4', color: '#14532d' }}>
            {image && (
            ...
            <div className="z-10 relative max-w-4xl space-y-6">
                <h1 className="text-6xl font-bold tracking-tight" style={{ color: '#15803d' }}>{title}</h1>
                <p className="text-2xl opacity-90" style={{ color: '#166534' }}>{subtitle}</p>
                <div className="w-32 h-2 rounded-full mx-auto mt-8" style={{ backgroundColor: '#86efac' }} />
            </div>
        </div>
    );
