...
const IntroductionSlideLayoutComponent = ({ data }: { data: z.infer<typeof Schema> }) => {
    ...
    return (
        <div className="w-full max-w-[1280px] aspect-video mx-auto relative overflow-hidden rounded-md h-full flex flex-row p-12" style={{ backgroundColor: '#f0fdf4', color: '#14532d' }}>
            <div className="w-1/3 border-r-4 pr-8 flex items-center justify-end" style={{ borderColor: '#86efac' }}>
                 <h2 className="text-5xl font-bold text-right" style={{ color: '#15803d' }}>{data.heading}</h2>
            </div>
