...
class PresentationJobModel(SQLModel, table=True):
    """
    Tracks background presentation generation jobs.
    Used for polling-based approach where the client starts a job and polls for status.
    This is an alternative to SSE streaming for presentation generation.
    """
    ...
    # Job status
    status: str = Field(default=PresentationJobStatus.PENDING.value)
    message: Optional[str] = None

    # Progress tracking
    progress_current: int = Field(default=0)  # Current slide being generated
    progress_total: int = Field(default=0)    # Total slides to generate
    progress_percentage: int = Field(default=0)

    # Error info
    error: Optional[dict] = Field(sa_column=Column(JSON), default=None)

    # Result data (presentation with slides when completed)
